...
namespace Chess
{
    public static class Zobrist
    {
        ...
        {
            //Debug.Log("     path "+ randomNumbersPath);
        ...
        /// Calculate zobrist key from current currentBoard position. This should only be used after setting currentBoard from fen; during search the key should be updated incrementally.
        public static ulong CalculateZobristKey(Board board)
        {
            ...
            for (int squareIndex = 0; squareIndex < 64; squareIndex++)
            {
                ...
                {
                    int pieceType = Piece.GetPieceTypeBinary(board.Square[squareIndex]);
                    int pieceColour = Piece.GetColor(board.Square[squareIndex]);
