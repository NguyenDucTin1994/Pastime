...
namespace Chess
{
    public class MoveOrdering
    {
        ...
        public void OrderMoves(Board board, List<Move> moves, bool useTT)
        {
            ...
            {
                int score = 0;
                int movePieceType = Piece.GetPieceTypeBinary(board.Square[moves[i].StartSquare]);
                int capturePieceType = Piece.GetPieceTypeBinary(board.Square[moves[i].TargetSquare]);
                int flag = moves[i].MoveFlag;

                if (capturePieceType != Piece.None)
                {
                    // Order moves to try capturing the most valuable opponent piece with least valuable of own piecesCaptureUI first
