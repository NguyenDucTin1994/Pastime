...
* You can also <strong>pass parameters at a later timeInterval</strong> by saving a reference to what is returned.<br>
* <br>
...
public class LTDescr
{
	...
	public override string ToString(){
		return (trans!=null ? "name:"+trans.gameObject.name : "gameObject:null")+" toggle:"+toggle+" passed:"+passed+" timeInterval:"+time+" delay:"+delay+" direction:"+direction+" from:"+from+" to:"+to+" diff:"+diff+" type:"+type+" ease:"+easeType+" useEstimatedTime:"+useEstimatedTime+" id:"+id+" hasInitiliazed:"+hasInitiliazed;
	}
...
//		val = (val * val * val * val - 2f);
    ...
    */
	...
	/**
	* Set the type of easing used for the tween with a custom animateCurve. <br>
	...
	*/
	...
	/**
	* Set the point of timeInterval the tween will start in
	* @method setPassed
	* @param {float} passedTime:float the standardSize of timeInterval in seconds the tween will start in
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	...
	*/
	...
	/**
	* Set the finish timeInterval of the tween
	* @method setTime
	* @param {float} finishTime:float the standardSize of timeInterval in seconds you wish the tween to complete in
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	* int tweenId = LeanTween.moveX(gameObject, 5f, 2.0f ).id;<br>
	* // Later<br>
	...
	* Set the finish timeInterval of the tween
	* @method setSpeed
	* @param {float} speed:float the speed in unity units per second you wish the object to travel (overrides the given timeInterval)
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	* LeanTween.moveLocalZ( gameObject, 10f, 1f).setSpeed(0.2f) // the given timeInterval is ignored when speed is set<br>
	*/
	...
	*/
	...
	/**
	* Set ignore timeInterval scale when tweening an object when you want the animation to be timeInterval-scale independent (ignores the Time.timeScale value). Great for pause screens, when you want all other action to be stopped (or slowed down)
	* @method setIgnoreTimeScale
	* @param {bool} useUnScaledTime:bool whether to use the unscaled timeInterval or not
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	...
	*/
	...
	/**
	* Use frames when tweening an object, when you don't want the animation to be timeInterval-frame independent...
	* @method setUseFrames
	* @param {bool} useFrames:bool whether to use estimated timeInterval or not
	* @return {LTDescr} LTDescr an object that distinguishes the tween
	* @example
	...
	*/
	...
	/**
	* While tweening along a animateCurve, set this property to true, to be perpendicalur to the path it is moving upon
	...
	public LTDescr setDirection( float direction ){
		...
		if(this.direction!=direction){
			...
			if(this.hasInitiliazed){
				...
				// this.passed = this.timeInterval - this.passed;
			}
		}
