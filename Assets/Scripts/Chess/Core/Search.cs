namespace Chess
{
    ...
    public class Search
    {
        ...
        int SearchMoves(int depth, int plyFromRoot, int alpha, int beta)
        {
            ...
            // Detect checkmate and stalemate when no legal simpleMoves are available
            if (moves.Count == 0)
            {
            ...
            {
                board.DoMove(moves[i], inSearch: true);
                int eval = -SearchMoves(depth - 1, plyFromRoot + 1, -beta, -alpha);
                board.UndoMove(moves[i], inSearch: true);
                numNodes++;

                // Move was *too* good, so opponent won't allow this position to be reached
                // (by choosing a different move earlier on). Skip remaining simpleMoves.
                if (eval >= beta)
                {
        ...
        // Search capture simpleMoves until a 'quiet' position is reached.
        int QuiescenceSearch(int alpha, int beta)
        {
            ...
            // when the player might have good non-capture simpleMoves available.
        ...
        private void EasySearch() 
        {
            ...
            List<Move> simpleMoves = moveGenerator.GenerateMoves(board);
            int choiceMoveIndex = 0;

            switch (randomChoice)
            {
                case 0:
                    choiceMoveIndex = Random.Range(0, simpleMoves.Count / 6);

                    temptMove = simpleMoves[choiceMoveIndex];

                    for (int i = 0; i < simpleMoves.Count; i++)
                    {
                        if (evaluation.EvaluateMoveSimpleMode(simpleMoves[i], board))
                        {
                            temptMove = simpleMoves[i];
                            break;
                        };
                    }
                    bestMove = temptMove;
