...
public class GeneralAdvancedTechniques : MonoBehaviour {
	...
	void Start () {
		...
		for(int i = 0; i < avatarSpeed.Length; i++)
			LeanTween.moveLocalZ( avatarSpeed[i], (i+1)*5f, 1f).setSpeed(6f).setEase(LeanTweenType.easeInOutExpo).setLoopPingPong(); // any timeInterval you set the speed it overrides the timeInterval value
