...
namespace Chess
{
    ...
    public static class PrecomputedMoveData
    {
        ...
        // Stores number of moves available in each of the 8 directions for every square on the currentBoard
        // Order of directions is: N, S, W, E, NW, SE, NE, SW
        // So for example, if availableSquares[0][1] == 7...
        // that means that there are 7 squares to the north of b1 (the square with index 1 in currentBoard array)
        public static readonly int[][] numSquaresToEdge;

        // Stores array of indices for each square a knight can land on from any square on the currentBoard
        ...
        static PrecomputedMoveData()
        {
            ...
            // Calculate knight jumps and available squares for each square on the currentBoard.
            ...
            for (int squareIndex = 0; squareIndex < 64; squareIndex++)
            {
                ...
                foreach (int knightJumpDelta in allKnightJumps)
                {
                    ...
                    if (knightJumpSquare >= 0 && knightJumpSquare < 64)
                    {
                        ...
                        // Ensure knight has moved max of 2 squares on x/y axis (to reject indices that have wrapped around side of currentBoard)
                ...
                foreach (int kingMoveDelta in directionOffsets)
                {
                    ...
                    if (kingMoveSquare >= 0 && kingMoveSquare < 64)
                    {
                        ...
                        // Ensure king has moved max of 1 square on x/y axis (to reject indices that have wrapped around side of currentBoard)
