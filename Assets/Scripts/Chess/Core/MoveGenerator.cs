    ...
    public class MoveGenerator
    {
        ...
        void GenerateKingMoves()
        {
            for (int i = 0; i < kingMoves[friendlyKingSquare].Length; i++)
            {
                ...
                // Skip squares occupied by friendly piecesCaptureUI
                if (Piece.CompareColor(pieceOnTargetSquare, friendlyColour))
                {
                    continue;
                }

                bool isCapture = Piece.CompareColor(pieceOnTargetSquare, opponentColour);
                if (!isCapture)
                {
        ...
        void GenerateSlidingPieceMoves(int startSquare, int startDirIndex, int endDirIndex)
        {
            ...
            for (int directionIndex = startDirIndex; directionIndex < endDirIndex; directionIndex++)
            {
                ...
                for (int n = 0; n < numSquaresToEdge[startSquare][directionIndex]; n++)
                {
                    ...
                    if (Piece.CompareColor(targetSquarePiece, friendlyColour))
                    {
                        break;
                    }
        ...
        void GenerateKnightMoves()
        {
            ...
            for (int i = 0; i < myKnights.Count; i++)
            {
                ...
                for (int knightMoveIndex = 0; knightMoveIndex < knightMoves[startSquare].Length; knightMoveIndex++)
                {
                    ...
                    int targetSquarePiece = board.Square[targetSquare];
                    bool isCapture = Piece.CompareColor(targetSquarePiece, opponentColour);
                    if (genQuiets || isCapture)
                    {
                        // Skip if square contains friendly piece, or if in check and knight is not interposing/capturing checking piece
                        if (Piece.CompareColor(targetSquarePiece, friendlyColour) || (inCheck && !SquareIsInCheckRay(targetSquare)))
                        {
                            continue;
                        }
                        moves.Add(new Move(startSquare, targetSquare));
            ...
            for (int i = 0; i < myPawns.Count; i++)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    ...
                    if (numSquaresToEdge[startSquare][pawnAttackDirections[friendlyColourIndex][j]] > 0)
                    {
                        ...
                        // Regular capture
                        if (Piece.CompareColor(targetPiece, opponentColour))
                        {
        ...
        bool IsPinned(int square) //a pinned piece is a piece that cannot move avoid A King is captured
        {
        ...
        {
            GenSlidingAttackMap();
            // Search squares in all directions around friendly king for checks/pins by enemy sliding piecesCaptureUI (queen, rook, bishop)
            int startDirIndex = 0;
            int endDirIndex = 8;
            ...
            for (int dir = startDirIndex; dir < endDirIndex; dir++)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    if (piece != Piece.None)
                    {
                        if (Piece.CompareColor(piece, friendlyColour))
                        {
                        ...
                        else
                        {
                            ...
                            // Check if piece is in bitmask of piecesCaptureUI able to move in current direction
        ...
        {
            // Update currentBoard to reflect en-passant capture
