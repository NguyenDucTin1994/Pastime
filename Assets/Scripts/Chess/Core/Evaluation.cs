...
namespace Chess
{
    public class Evaluation
    {
        ...
        public int EvaluateEasyMode(Board board)
        {
        ...
        int CountMaterial(int colourIndex) // Total score of 1 Player (black or white)
        {
            int material = 0;
        ...
        int EvaluatePieceSquareTables(int colourIndex, float endgamePhaseWeight) //Total value +- because current position
        {
            int value = 0;
            ...
            //value += PieceSquareTable.Read (PieceSquareTable.kingMiddle, currentBoard.KingSquare[colourIndex], isWhite);

            return value;
        }

        static int EvaluatePieceSquareTable(int[] table, PieceList pieceList, bool isWhite) //  value +- because  current position
        {
            int value = 0;
        ...
        static int EvaluatePiece(PieceType pieceTyoe)
        {
            switch (pieceTyoe)
            {
                case PieceType.Pawn:
        ...
        public  bool EvaluateMoveSimpleMode(Move move, Board board) 
        {

            int startPiece = board.Square[move.StartSquare];
            int startValue = EvaluatePiece(Piece.GetPieceType(startPiece));
            var startColor = Piece.GetPieceColor(startPiece);

            int endPiece = board.Square[move.TargetSquare];
            int endValue = EvaluatePiece(Piece.GetPieceType(endPiece));
            var endColor = Piece.GetPieceColor(endPiece);
